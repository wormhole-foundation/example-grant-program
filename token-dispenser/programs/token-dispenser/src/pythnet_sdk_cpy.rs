//taken from pythnet sdk and adjusted for anchor 0.29
// see https://github.com/pyth-network/pyth-crosschain/tree/main/pythnet/pythnet_sdk
...
#[derive(
    Debug, Clone, PartialEq, Eq, AnchorSerialize, AnchorDeserialize, Default,
)]
...
mod test {
    ...
    fn test_keccak256() {
        ...
        let hash_a = HasherKeccak256::hashv(&[data]);

        let data = [b"hello", b"world"];
        let hash_b = HasherKeccak256::hashv(&data);

        assert_eq!(hash_a, hash_b);
    }

    #[derive(Default, Clone, Debug, AnchorSerialize)]
    struct PriceAccount {
        pub id:         u64,
        pub price:      u64,
        pub price_expo: u64,
    ...
    struct MerkleTreeDataWrapper {
        pub accumulator: MerkleTree<HasherKeccak256>,
    ...
    impl Arbitrary for MerkleTreeDataWrapper {
        ...
        fn arbitrary_with(size: Self::Parameters) -> Self::Strategy {
            ...
            .prop_map(|v| {
                ...
                let accumulator =
                    MerkleTree::<HasherKeccak256>::from_set(data.iter().map(|i| i.as_ref())).unwrap();
    ...
    impl Arbitrary for MerklePath<HasherKeccak256> {
        type Parameters = usize;
    ...
    fn test_merkle() {
        ...
        let item_a = AnchorSerialize::try_to_vec(&price_account_a).unwrap();

        let mut price_only_b = PriceOnly::from(price_account_a);
        price_only_b.price = 200;
        let item_b = AnchorSerialize::try_to_vec(&price_only_b).unwrap();
        ...
        let accumulator = MerkleTree::<HasherKeccak256>::from_set(set.into_iter()).unwrap();
        let proof = accumulator.prove(&item_a).unwrap();

        assert!(accumulator.verify_path(proof, &item_a));
        let proof = accumulator.prove(&item_a).unwrap();
        assert_eq!(size_of::<<HasherKeccak256 as Hasher>::Hash>(), 32);
    ...
    fn test_merkle_default_proof_fails() {
        ...
        let accumulator = MerkleTree::<HasherKeccak256>::from_set(set.into_iter()).unwrap();
        let proof = MerklePath::<HasherKeccak256>::default();
        assert!(!accumulator.verify_path(proof, &item_a));
        let proof = MerklePath::<HasherKeccak256>(vec![Default::default()]);
        assert!(!accumulator.verify_path(proof, &item_a));
    }
    ...
    fn test_merkle_second_preimage_attack() {
        ...
        // H(a || b) at the leaf.
        let faulty_accumulator = MerkleTree::<HasherKeccak256> {
        ...
        let fake_leaf = &[
            MerkleTree::<HasherKeccak256>::hash_leaf(&item_b),
            MerkleTree::<HasherKeccak256>::hash_leaf(&item_a),
        ]
        .concat();

        // Confirm our combined hash existed as a node pair in the original tree.
        assert_eq!(
            MerkleTree::<HasherKeccak256>::hash_leaf(fake_leaf),
            accumulator.nodes[2]
        );
    ...
    proptest! {
        ...
        fn test_fake_merkle_proofs(
            ...
            p in any::<MerklePath<HasherKeccak256>>(),
        ) {
