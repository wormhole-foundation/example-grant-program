...
import { checkTransactions, deserializeTransactions } from "../utils/fundTransactions";
...
export const fundTransaction = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
...
async function loadFunderWallet(): Promise<NodeWallet> {
  const secretData = await getSecret(process.env.FUNDER_WALLET_KEY_SECRET_NAME ?? "xli-test-secret-funder-wallet");
