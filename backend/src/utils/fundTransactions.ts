import { ComputeBudgetProgram, Ed25519Program, PublicKey, Secp256k1Program, VersionedTransaction } from "@solana/web3.js";
import { getSecret } from "./index";

const SET_COMPUTE_UNIT_LIMIT_DISCRIMINANT = 2;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function deserializeTransactions(transactions: any): VersionedTransaction[] {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
async function loadTokenDispenserProgramId(): Promise<string> {
  const secretData = await getSecret(process.env.TOKEN_DISPENSER_PROGRAM_ID_SECRET_NAME ?? "xli-test-secret-token-dispenser-program-id");
...
async function loadWhitelistedProgramIds(): Promise<PublicKey[]> {
  ...
  return [PROGRAM_ID, Secp256k1Program.programId, Ed25519Program.programId, ComputeBudgetProgram.programId];
}

export function checkAllProgramsWhitelisted(transaction: VersionedTransaction, whitelist: PublicKey[]): boolean {
  for (const ix of transaction.message.compiledInstructions) {
    if (!whitelist.some(program => transaction.message.staticAccountKeys[ix.programIdIndex].equals(program))) {
      return false;
    }
  }
  return true;
...
export function checkSetComputeBudgetInstructionsAreSetComputeUnitLimit(transaction: VersionedTransaction) {
  for (const ix of transaction.message.compiledInstructions) {
    if (transaction.message.staticAccountKeys[ix.programIdIndex].equals(ComputeBudgetProgram.programId)) {
...
export function checkProgramAppears(transaction: VersionedTransaction, program: PublicKey): boolean {
  for (const ix of transaction.message.compiledInstructions) {
    if (transaction.message.staticAccountKeys[ix.programIdIndex].equals(program)) {
      return true;
    }
  }
  return false;
...
export function countTotalSignatures(transaction: VersionedTransaction): number {
  return (
...
export function countPrecompiledSignatures(transaction: VersionedTransaction, program: PublicKey): number {
  return transaction.message.compiledInstructions
    .filter(ix => {
      return transaction.message.staticAccountKeys[ix.programIdIndex].equals(program);
    })
    .reduce((acc, ix) => acc + ix.data[0], 0);
}

// TODO: Verify if this is the expected behavior
export function checkNumberOfSignatures(transaction: VersionedTransaction): boolean {
  return countTotalSignatures(transaction) <= 3;
}

export function checkTransaction(transaction: VersionedTransaction, tokenDispenser: PublicKey, whitelist: PublicKey[]): boolean {
...
export async function checkTransactions(transactions: VersionedTransaction[]): Promise<boolean> {
  const whitelist = await loadWhitelistedProgramIds();

  return transactions.every(tx => checkTransaction(tx, whitelist[0], whitelist));
}
