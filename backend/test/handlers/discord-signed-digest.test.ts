...
import { DiscordSignedDigestRequest, signDiscordMessage } from "../../src/handlers/discord-signed-digest";
...
const givenDownstreamServicesWork = () => {
  server.use(
    ...
    http.all("https://secretsmanager.us-east-2.amazonaws.com", () => {
      return HttpResponse.json({ SecretString: JSON.stringify({ target: [...new Keypair().secretKey] }) });
    }),
  );
  server.listen();
};
...
const whenSignDiscordMessageCalled = async (body: DiscordSignedDigestRequest) => {
