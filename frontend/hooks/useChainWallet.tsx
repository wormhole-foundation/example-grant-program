...
export function ChainWalletProvider({ children }: ChainWalletProviderProps) {
  ...
  const updateAddress = useCallback((address: string | undefined, chainName: string) => {
...
export function useChainWallet(chainName: ChainName, walletName = WALLET_NAME) {
    ...
    const walletRepo = useMemo(() => manager.getWalletRepo(chainName), [manager, chainName])
    const wallet = useMemo(() => walletRepo.getWallet(walletName), [walletRepo, walletName])
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const isWalletConnecting = useMemo(() => wallet?.walletStatus === 'Connecting', [wallet, _])
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const isWalletConnected = useMemo(() => wallet?.walletStatus === 'Connected', [wallet, _])
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const isWalletDisconnected = useMemo(() => wallet?.walletStatus === 'Disconnected', [wallet, _])
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const isWalletNotExist = useMemo(() => wallet?.walletStatus === 'NotExist', [wallet, _])
